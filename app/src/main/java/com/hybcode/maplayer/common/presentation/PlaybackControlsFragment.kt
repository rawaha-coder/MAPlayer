...
// get information about the currently playing song
        musicLibraryViewModel.currentlyPlayingSong.observe(viewLifecycleOwner) { song ->
            song?.let {
...
// check whether a song is currently playing
        musicLibraryViewModel.isPlaying.observe(viewLifecycleOwner) { playing ->
            playing?.let {
...
// keep track of currently playing song duration
        musicLibraryViewModel.currentPlaybackDuration.observe(viewLifecycleOwner) { duration ->
            duration?.let {
...
// keep track of currently playing song position
        musicLibraryViewModel.currentPlaybackPosition.observe(viewLifecycleOwner) { position ->
            position?.let {
